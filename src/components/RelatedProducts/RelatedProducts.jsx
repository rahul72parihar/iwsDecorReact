    ...
    <div className="rp-stars" aria-label={`Rating ${safe.toFixed(1)} out of 5`}>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        return (
          <span
            key={i}
            className={`rp-star ${isFull ? 'full' : isHalf ? 'half' : 'empty'}`}
          >
            ★
          </span>
        );
...
export default function RelatedProducts({ currentProduct }) {
  ...
  const related = useMemo(() => {
    ...
    const start =
      Number(currentProduct.id) % Math.max(1, pool.length);
  ...
  const onWishlistToggle = (product) => {
    setWish((prev) => ({
      ...prev,
      ...
      {related.map((p) => (
        <div className="rp-card" key={p?.id}>
          ...
          <div className="rp-ratingRow">
            ...
            <div className="rp-reviewCount">
              ({(p?.reviewsCount || 0).toLocaleString('en-IN')})
            </div>
          </div>
